...
1. Create malicious GitHub App — T1585.004
2. Install on attacker-controlled repo — T1078
3. Open issue/PR to bypass permission check — T1190
4. Inject malicious prompt via issue content — T1059
5. Read `/proc/self/environ` for secrets — T1552.001
6. Exchange tokens for privileged GitHub access — T1528
7. Push backdoored code to Anthropic's repo — T1195.002
...
### Least Privilege
The core issue in this attack is that permissions were broader than they needed to be. The `[bot]` actor was trusted unconditionally — it was given more access than its role required. A least privilege approach asks: *what is the minimum access this account actually needs to do its job?* Applying that question to automated workflows and bot accounts would have limited the blast radius significantly, even if the bypass still occurred.
...
*Analyzed by Beth Watt | Cybersecurity Transition Portfolio*  
